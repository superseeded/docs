...
sequenceDiagram
    ...
    U-->>M: Return file URL
    M->>S: POST /v1/resolve/specs/from-upload
...
<Steps>
  ...
  <Step title="Call resolve-specs">
    Your frontend calls `POST /v1/resolve/specs/from-upload` with the same delegation token and the file URL. The API fetches the file internally and processes it.
  </Step>
