...
sequenceDiagram
    ...
    T-->>F: Return file URL
    F->>A: POST /v1/verify/pot-sizes (or other processing route)
...
<Steps>
  ...
  <Step title="Call processing endpoint">
    Your frontend calls a processing endpoint (e.g., `POST /v1/verify/pot-sizes` or `POST /v1/verify/plant-names`) with the file URL. The API fetches the file internally and processes it.
  </Step>
