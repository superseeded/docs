...
SuperSeed.init({
  ...
  onComplete: (result) => {
    ...
    // Call the resolve-specs endpoint with the file URL to process and get enriched data
  },
...
| `onComplete` | `{ uploadId, fileName, fileUrl }` | Upload finished successfully. Use `fileUrl` to call `/v1/resolve/specs/from-upload` |
...
<script>
  const uploader = SuperSeed.init({
    ...
    onComplete: (result) => {
      ...
      // Call /v1/resolve/specs/from-upload to process and get enriched data
    }
  });
  ...
  <script>
    ...
    SuperSeed.init({
      ...
      onComplete: async (result) => {
        // Call resolve-specs with the file URL to process and get enriched data
