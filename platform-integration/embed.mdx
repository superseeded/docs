...
| `onComplete` | `{ uploadId, fileName, fileUrl }` | Upload finished successfully. Use `fileUrl` to call `/v1/resolve/specs` |
...
<script>
  const uploader = SuperSeed.init({
    ...
    onComplete: (result) => {
      ...
      // Call /v1/resolve/specs to process and get enriched data
    }
  });
