...
- **Resolve Specifications** - Classify pot sizes and plant specifications (JSON files only)
...
<Note>
Merchant ID is configured in settings only and cannot be overridden per-shortcode for security reasons.
</Note>
...
| `superseeded:complete` | `{ id, uploadURL, fileUrl, name }` |
| `superseeded:error` | `{ id, error }` |
| `superseeded:server-notified` | `{ success, message, data }` |
| `superseeded:specs-resolved` | `{ id, fileUrl, fileName, specsData }` |
| `superseeded:route-complete` | `{ fileId, route, result }` |
| `superseeded:route-error` | `{ fileId, route, error }` |
| `superseeded:routes-complete` | `{ fileId, results, successful, failed }` |
...
Proxy for the SuperSeeded resolve-specs API. Use this to process items and retrieve enriched data.
...
{
  "items": [
    { "brand": "Bosch", "mpn": "GWS18V-45" },
    { "brand": "Makita", "mpn": "XAG04Z" }
  ]
}
```
...
<Note>
For JSON file uploads, the widget automatically calls the API directly using the delegation token. The `superseeded:specs-resolved` event fires when enrichment completes.
</Note>
...
SuperSeededWordPress.executeRoute(routeConfig, fileUrl, token, apiBaseUrl)
    .then(function(data) {
