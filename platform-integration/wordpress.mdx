...
<Note>
For file uploads, the widget automatically calls the `/v1/resolve/specs` endpoint directly using the delegation token and the file URL. The `superseeded:specs-resolved` event fires when enrichment completes.
</Note>
