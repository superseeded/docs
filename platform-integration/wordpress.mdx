...
| `superseeded:specs-resolved` | `{ id, fileUrl, fileName, specsData }` - fired when resolve-specs route completes |
...
<Note>
For file uploads, the widget automatically calls the `/v1/resolve/specs/from-upload` endpoint directly using the delegation token and the file URL. The `superseeded:specs-resolved` event fires when enrichment completes.
</Note>
...
Fires when the resolve-specs API returns enriched data from a processed request via the WordPress proxy endpoint.
...
// Call resolve-specs via WordPress proxy (for manual item arrays)
...
// Call resolve-specs directly using delegation token and file URL
