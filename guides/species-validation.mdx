...
The API queries 11 botanical databases by default and consolidates authoritative results into one unified taxonomic record. Add `apc_apni` to the request's `sources` list when versioned Australian name resolution is required.

<Steps>
  <Step title="Parallel Database Search">
    The 11 default taxonomic databases are queried simultaneously for each name. Explicit `sources` lists are honored, including `apc_apni`.
  </Step>
