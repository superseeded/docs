...
This endpoint queries APC/APNI and 11 botanical databases in parallel, consolidates authoritative results, and returns a unified taxonomic record per input. Exact APC accepted names and explicit synonym links can validate independently. Fuzzy, APNI-only, split, and conflicting results remain uncertain until corroborated or reviewed. Names that no authoritative source resolves may receive a Gemini suggestion, but AI never participates in consensus or species identity.

Sources queried: APC/APNI, GBIF, TNRS (WFO), POWO (Kew), iNaturalist, EOL, ALA, IP Australia PBR, UPOV, WikiSpecies, Wikidata, Wikipedia.

### Australian name resolution

For names found in APC or APNI, SuperSeeded uses an [open-source Rust port of APCalign](https://github.com/superseeded/apcalign-rs). It follows APCalign's ordered approach to accepted names, synonyms, spelling variants, authorship, infraspecific ranks, hybrids, phrase names, and conservative typo correction against a fixed backbone release.

This makes the result predictable and reviewable: exact APC accepted names and explicit synonym links can validate directly, while spelling corrections, APNI-only names, taxonomic splits, and ambiguous matches set `review_required: true`. Check `alternatives` before selecting a taxon when review is required. The `backbone_version` and `resolver_version` fields identify the data and resolver versions used for the result.
...
<ParamField body="sources" type="string[]">
  Specific authoritative sources to query. Defaults to APC/APNI plus all 11 existing non-Gemini sources. Explicit lists are honored. Gemini is not a consensus source.
